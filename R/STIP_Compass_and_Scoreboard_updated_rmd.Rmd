...
stip_0 <- read_delim('../data//STIP_Survey.csv', '|', escape_double = FALSE, trim_ws = TRUE)
```
...
stip_instrument <- stip_0[,grepl('^[F][0-9]', (names(stip_0)))] %>% cbind(stip_0[,!grepl('Instrument', (names(stip_0)))])
```

```{r}
# Removes all columns with instrument (start with F)
data <- stip_0[,!grepl('^[F][0-9]', (names(stip_0)))]

#There are a few more columns with information on instruments. We remove them, too for the sake of coherence
data <- data[,!grepl('Instrument', (names(data)))]
