...
data <- read_csv("Data/results_all.csv", show_col_types = FALSE)
...
```{r}
#data <- data %>% select(COUNTRY, YEAR, INDICATOR_LIST)
```
...
```{r}
index1 <- c("B_XGDP", "GV_XGDP", "G_XGDP", "GV_XFB")
index2 <- c("G_FBXGDP", "H_XFB", "COPAT_XPCT", "B_XFG")
index3 <- c("GV_XGDP", "G_XGDP")
index4 <- c("COPAT_XPCT", "B_XFG")
index5 <- c("H_XFB", "G_FBXGDP")
```
...
Doing a hierarchical clustering based on the first 2 principal components
...
```{r}
summary_df %>% filter(Clust_1 >= 1) %>% arrange(desc(Clust_1))
```
...
stat <- cbind(stat, rowMeans(stat[,3:9])) %>% select(-COUNTRY)

stat <- stat %>% rename("RowMeans" = `rowMeans(stat[, 3:9])`)
```
...
```{r}
index1_df <- data_all %>% as_tibble() %>% select(index1, cluster)
index1_df <- index1_df %>% mutate(I1 = rowMeans(index1_df[,1:4]))
index1
```

```{r}
index1_df %>% group_by(cluster) %>% summarise(n = n(), avg = sum(I1) / n)

```


```{r}
index2_df <- data_all %>% as_tibble() %>% select(index2, cluster)
index2_df <- index2_df %>% mutate(I2 = rowMeans(index2_df[,1:3]))
index2
```

```{r}
index2_df %>% group_by(cluster) %>% summarise(n = n(), avg = sum(I2) / n)
```

```{r}
index3_df <- data_all %>% as_tibble() %>% select(index3, cluster)
index3_df <- index3_df %>% mutate(I3 = rowMeans(index3_df[,1:2]))
index3
```

```{r}
index3_df %>% group_by(cluster) %>% summarise(n = n(), avg = sum(I3) / n)
```

```{r}
index4_df <- data_all %>% as_tibble() %>% select(index4, cluster)
index4_df <- index4_df %>% mutate(I4 = rowMeans(index4_df[,1:2]))
index4
```

```{r}
index4_df %>% group_by(cluster) %>% summarise(n = n(), avg = sum(I4) / n)
```

```{r}
index5_df <- data_all %>% as_tibble() %>% select(index5, cluster)
index5_df <- index5_df %>% mutate(I5 = rowMeans(index5_df[,1:2]))
index5
```

```{r}
index5_df %>% group_by(cluster) %>% summarise(n = n(), avg = sum(I5) / n)
```
